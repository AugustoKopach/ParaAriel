...
class HardwareManagementCLIApp(cmd.Cmd):
    ...
    def preloop(self):
        ...
        self.kernel = Kernel()  # Crea una instancia de Kernel
        print("Kernel instance created:", self.kernel)  # Agrega esta línea para depurar
    ...
    def do_kill(self, line):
        pid = int(line)  # Convertir a entero
    ...
    #     # Load a program from the ones in SoftwareProgram
    #     try:
    ...
    #         for i in range(0, len(prog.instructions)):
    #             HARDWARE.memory.write(i, prog.instructions[i])
    #     except:
    ...
    # Load a program from the ones in SoftwareProgram
        try:
            ...
            self.kernel.create_process(prog)
            # LoPrinter.show("Program loaded into memory. Memory range: {}-{}".format(memory_start, memory_end))
        except KeyError:
            Printer.error("No program with the name: " + line)
